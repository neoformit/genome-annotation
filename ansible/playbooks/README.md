...
- **`playbook-install-apollo-jbrowse1-plugins.yml`**
    Install custom JBrowse1 plugins (declared per host in `host_vars/<apollo>/vars` as `apollo_jbrowse1_plugins`) into the JBrowse embedded inside Apollo's WAR. Does not redeploy the WAR — safe to run against a live production Apollo. The same install step is also wired into `playbook-build-nectar-apollo.yml` under the `deploy` tag, so a normal Apollo build/redeploy re-establishes plugins automatically.

    **Inventory:** `hosts` (limit to the target Apollo, e.g. `--limit apollo-037.genome.edu.au`).

    See [jbrowse1-plugins-on-apollo.md](jbrowse1-plugins-on-apollo.md) for full details, including the per-organism `trackList.json` registration that Apollo also requires.
